...
class AttentionModules(tf.keras.layers.Layer):
    """
                               -- Attention Modules --
    'attention module' includes channel attention and spatial attention modules beside on :
    ...
    def attention_module(self, **params):
        ...
        def attention_module(decoder, encoder):
            ...
            mlp_0 = tf.keras.layers.Dense(units=filters // ratio, activation=tf.keras.layers.Activation(activation='relu'))(
                decoder_input)
            mlp_1 = tf.keras.layers.Dense(units=filters)
            avg_ = mlp_1(mlp_0)
            avg_ = tf.keras.activations.relu(avg_)
            channel_output = tf.keras.layers.multiply([avg_, encoder])
    ...
    def self_attention_module(self, **params):
        ...
        def self_attention_module(inputs):
            ...
            query = tf.keras.layers.Conv2D(filters=filters // ratio, kernel_size=(1,1),
                                           padding=padding, kernel_initializer=kernel_initializer)(inputs)
            key = tf.keras.layers.Conv2D(filters=filters // ratio, kernel_size=(1,1),
                                         padding=padding, kernel_initializer=kernel_initializer)(inputs)
            value = tf.keras.layers.Conv2D(filters=filters, kernel_size=(1,1), padding=padding,
                                           kernel_initializer=kernel_initializer)(inputs)
            image_dims = tf.shape(value)
            batch_size = image_dims[0]
            height = image_dims[1]
            width = image_dims[2]
            channels = image_dims[3]

            proj_query = tf.reshape(query, [batch_size, height * width, channels // ratio])
            proj_key = tf.transpose(tf.reshape(key, [batch_size, height * width, channels // ratio]), (0, 2, 1))
